...
def voltagePattern(V_gain, time, t_start, t_stop, shape):
    for r in range (1,5):
        ...
        if t_start + r/F < time < t_stop + r/F:
            if shape == 'square':
                return V_gain
            if shape == 'prosthetic':
                return V_gain*(1-numpy.exp(-(time-t_start-r/F)/2.0))
            if shape == 'triangle':
                return V_gain/(t_stop-t_start)*(time-t_start-r/F)
        if r/F+t_stop < time < (r+1)/F + t_start:
            if shape == 'square':
                return 0.0
            if shape == 'prosthetic':
                return V_gain*(1-numpy.exp(-(t_stop-t_start)/2.0))*numpy.exp(-(time-t_stop-r/F)/12.0)
            if shape == 'triangle':
                return max(-V_gain/(t_stop-t_start)*(time-(t_start+t_stop)), 0.0)
    else:
        return 0.0
