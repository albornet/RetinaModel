...
neuronParams      = {'V_th': threshPot, 't_ref': 0.75, 'V_reset': -67.5, 'a': 0.0, 'b': -50.0, 'tau_w': 400.0}
    ...
    for i in range(nRows):
        for j in range(nCols):
            ...
            if distance < inputRadius:
                    ...
                    if len(nest.GetStatus([GCSD[target]], keys='events')[0]['times']) < 25 :
                        StimGC    = input_GC[t]*inputSpaceFrame(distance, 0.5*inputRadius)
                        nest.SetStatus([GC[target]], {'V_m': restPot + GCVoltage + StimGC})
                    else:
                        if t_tired[target] == None:
                            t_tired[target] = t
                            u_tired[target] = GCVoltage + restPot
                        nest.SetStatus([GC[target]], {'V_m': restPot + (u_tired[target] - restPot)*numpy.exp(-(t-t_tired[target])*stepDuration/tau_tired)})
                        #print('coucouphoque')
