...
input_GC        = inputTimeFrame(RC_GC, 0.9*inputVoltage, inputNoise, time, startTime + delayGC, stopTime + delayGC, shape)
...
interneuronModel  = 'iaf_cond_alpha'
                                                                       # a qui ne fait rien???
neuronParams      = {'V_th': threshPot, 't_ref': 2.0}
...
BC2 = nest.Create(interneuronModel, BGCRatio*nRows*nCols, interNeuronParams)
...
BC2LastVoltage = numpy.zeros((len(BC2),))
...
for i in range(len(neuronsToRecord)):
    ...
    BC2MM = nest.Create('multimeter', params = {'withtime': True, 'interval': 0.01, 'record_from': ['V_m']})
    nest.Connect(BC2MM, [BC2[0*nRows*nCols + neuronsToRecord[i][0]*nCols + neuronsToRecord[i][1]]])
    BC2MMs.append(BC2MM)
        ...
        for j in range(nCols):
            ...
            if distance < inputRadius:
                ...
                for k in range(BGCRatio):
                    ...
                    BC2Voltage = nest.GetStatus([BC2[target]], 'V_m')[0]- restPot
                    nest.SetStatus([BC2[target]], {'V_m': restPot + BC2Voltage + StimBC})
    ...
    for i in range(nRows):
        for j in range(nCols):
            for kBC in range(BGCRatio):
                ...
                preSynVoltage2         = nest.GetStatus([BC2[source]], 'V_m')[0] - restPot
                deltaPreSynVoltage2    = (preSynVoltage2 - BC2LastVoltage[source])/stepDuration
                if deltaPreSynVoltage2 > 0.0:
                    postSynVoltage    = nest.GetStatus([GC[target]], 'V_m')[0] - restPot
                    nest.SetStatus([GC[target]], {'V_m': restPot + postSynVoltage + connections['BC2_To_GC']*preSynVoltage2})
    ...
    for i in range(nRows):
        for j in range(nCols):
            for k in range(BGCRatio):
                ...
                BC2LastVoltage[source] = nest.GetStatus([BC2[source]], 'V_m')[0] - restPot
            for k in range(AGCRatio):
                source = k*nRows*nCols + i*nCols + j
...
for i in range(len(neuronsToRecord)):
    ...
    events = nest.GetStatus(BC2MMs[i])[0]['events']
    tPlot  = events['times'];
    ...
    # Do the rasterplot
    plt.subplot(5, len(neuronsToRecord)+1, 5*(len(neuronsToRecord)+1)+i+1)
...
f.close()
plt.subplot(5, len(neuronsToRecord)+1, 6*(len(neuronsToRecord)+1))
